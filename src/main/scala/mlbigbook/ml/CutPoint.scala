...
object CutPoint {
  ...
  def apply[D[_]: Data, N: Numeric](data: D[(N, Boolean)])(
    ...
    if (data isEmpty)
      ...
      val totalEntropy = Entropy.entropy(data)
      ...
      val cpWithInfoIncrease =
        allCutPoints
          .map { cp =>
            ...
            val scaledS1Entropy = (s1.size / totalSize) * Entropy.entropy(s1)
            val scaledS2Entropy = (s2.size / totalSize) * Entropy.entropy(s2)
  ...
  def partition[D[_]: Data, N: Numeric](data: D[N], cp: N): (D[N], D[N]) = ???
