...
object OnlineMeanVariance {
  ...
  private[this] def fromLong[N: Numeric](l: Long): N = {
    throw new RuntimeException("ahhhhh cannot figure out Long => N : Numeric   :(")
  }

  /**
   * Uses Welford's Method to calculate variance.
   */
  def apply[N: Numeric, V[_] <: Vector[_]](elems: Data[V[N]])(implicit ops: VectorOpsT[N, V]): Stats[V[N]] =
      ...
      case Some(v) =>
            ...
            .aggregate(State(0l, ops.zeros(size), ops.zeros(size)))(
              {
                case (State(n, mean, m2), current) =>
                  ...
                  val newMean = ops.addV(mean, ops.divS(delta, fromLong(n)))
                ...
                case (State(n1, mean1, m21), State(n2, mean2, m22)) =>
                  ...
                  val newMean =
                    if (n2 * 10l < n1)
                      ...
                      ops.addV(mean1, ops.divS(ops.mulS(delta, fromLong(n2)), fromLong(newN)))
                    else if (n1 * 10 < n2)
                      //                      mean2 - (delta * n1) / (n1 + n2)
                      ops.subV(mean2, ops.divS(ops.mulS(delta, fromLong(n1)), fromLong(newN)))
                    else
                      //                      (mean1 * n1 + mean2 * n2) / (n1 + n2)
                      ops.divS(ops.addV(ops.mulS(mean1, fromLong(n1)), ops.mulS(mean2, fromLong(n2))), fromLong(newN))
                  ...
                  val newM = {
                    ...
                    val fraction =
                      ops.divS(
                        ops.mulS(ops.mulS(deltaSquared, fromLong(n1)), fromLong(n2)),
                        fromLong(newN)
                      )
        ...
        Stats(
          ...
          if (res.count < 2l)
            ...
            ops.divS(res.m2, fromLong(res.count - 1l))
        )
