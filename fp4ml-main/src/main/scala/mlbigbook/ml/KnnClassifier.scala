...
trait KnnClassifier extends ClassificationModule {
  ...
  private[this] lazy val nnRankMod = NearestNeighbors[(Item, Label), N, V](vops)
  ...
  def train[D[_]: Data](
    ...
    itemToClassify => {
      ...
      Argmax(votesForNeighborhood.toSeq)
        .fold { emptyLabel } { case (majorityLabel, _) => majorityLabel }
    }
  }
