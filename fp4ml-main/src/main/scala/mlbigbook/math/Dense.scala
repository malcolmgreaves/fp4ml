 ...
 * of MathVectorOps.
 */
protected abstract class Dense[@specialized Num: Fractional: Zero: Semiring: ClassTag]
  ...
  override final def map[B: ClassTag: Fractional: Zero](v: DenseVector[N])(f: N => B): DenseVector[B] =
    v.map(f)

  override final def reduce[B >: N: ClassTag](v: DenseVector[N])(r: (B, B) => B) =
    v.reduceLeft(r)

  override final def fold[B: ClassTag](v: DenseVector[N])(zero: B)(combine: (B, N) => B) =
    v.valuesIterator
